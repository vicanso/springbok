export default {
  themeSystem: "系统主题",
  themeDark: "深色主题",
  themeLight: "浅色主题",
  home: {
    file: "文件",
    size: "大小",
    savings: "节约空间",
    diff: "差异",
    again: "再次执行",
    start: "开始",
    dragDropTips: "拖动图片文件至以上区域放开",
    savingsTips:
      "节约大小: {savedSize}，总大小：{totalSize}。每个文件平均节约：{average}%（最高{top}%）",
    clean: "清除",
    outputDir: "选择输出目录",
    clearOutputDir: "清除输出目录",
    stripExif: "清除 EXIF / 元数据",
    notSupported: "暂未支持",
    selectImageFail: "选择图片失败",
    optimImageFail: "优化图片失败",
    restoreImageFail: "恢复图片失败",
  },
  setting: {
    disabled: "禁用",
    supportImages: "支持的图片格式",
    supportImageTips: "启用要支持的图片格式",
    concurrency: "并行任务数",
    concurrencyTips: "同时处理的文件数量，值越高速度越快，但占用 CPU 也越多。",
    presetHigh: "高质量",
    presetMedium: "均衡",
    presetLow: "高压缩",
    imageQuality: "图片压缩级别",
    imageQualityTips: "输入各图片格式的压缩质量级别",
    pngQualityPlaceholder: "输入png图片的压缩质量级别",
    jpegQualityPlaceholder: "输入jpeg图片的压缩质量级别",
    avifQualityPlaceholder: "输入avif图片的压缩质量级别",
    webpQualityPlaceholder: "输入webp图片的压缩质量级别",
    jxlQualityPlaceholder: "输入jxl图片的压缩质量级别",
    outputDir: "输出目录",
    outputDirTips: "将处理后的文件保存到指定目录，原文件保持不变。",
    outputDirPlaceholder: "与源文件同目录（原地修改）",
    selectOutputDir: "选择目录",
    clearOutputDir: "清除",
    reset: "重置配置",
    supportConverts: "支持转换的图片格式",
    supportConvertTips: "启用要支持的图片转换格式",
  },
};
